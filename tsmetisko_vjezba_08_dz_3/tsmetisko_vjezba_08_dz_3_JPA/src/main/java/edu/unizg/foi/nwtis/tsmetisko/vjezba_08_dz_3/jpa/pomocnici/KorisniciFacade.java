...
import edu.unizg.foi.nwtis.tsmetisko.vjezba_08_dz_3.jpa.entiteti.Korisnici_;
...
public class KorisniciFacade extends EntityManagerProducer implements Serializable {
  ...
  public List<String> dohvatiUlogu(String korisnik) {
    CriteriaQuery<String> cq = cb.createQuery(String.class);
    Root<Uloge> root = cq.from(Uloge.class);

    cq.select(root.get("grupa")).where(cb.equal(root.get("korisnik"), korisnik));

    TypedQuery<String> query = getEntityManager().createQuery(cq);
    List<String> rezultati = query.getResultList();
