...
.desno {
    ...
    <body>
       ...
       <ul>
            ...
            <br/>       
        <table>
        <tr><th>R.br. <th>Korisnik</th><th>Naziv</th><th>Adresa</th><th>Mrežna vrata</th><th>Mrežna vrata za kraj</th><th>Admin kod</th></tr>
	<%
	int i=0;
	...
	for(Partner p: partneri) {
	  i++;
	  %>
       <tr><td class="desno"><%= i %></td><td><%= p.id() %></td><td><%= p.naziv() %></td><td><%= p.adresa() %></td><td><%= p.mreznaVrata() %></td><td><%= p.mreznaVrataKraj() %></td><td><%= p.adminKod() %></td>
	       <td>
